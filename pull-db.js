...
if (PARAMS.length == 0 || CONFIGURATION['environments-list'].indexOf(PARAMS[0]) == -1) {
...
async function main() {
  ...
  const userCollections = collections.filter((x) => x.id == 'users').pop();
  const documents = await userCollections.listDocuments();
  console.log(documents.length);
  const collectionPath = `${folderName}/${userCollections.id}`;
